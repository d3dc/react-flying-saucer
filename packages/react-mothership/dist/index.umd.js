(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('react'), require('react-router'), require('react-redux'), require('../context'), require('../scope'), require('lodash/mapValues'), require('history'), require('../store'), require('../../helpers/esm/typeof'), require('lodash/partition'), require('../path'), require('../router'), require('react-router-dom'), require('lodash/get'), require('lodash/merge'), require('@rematch/core'), require('@rematch/select')) :
  typeof define === 'function' && define.amd ? define(['exports', 'react', 'react-router', 'react-redux', '../context', '../scope', 'lodash/mapValues', 'history', '../store', '../../helpers/esm/typeof', 'lodash/partition', '../path', '../router', 'react-router-dom', 'lodash/get', 'lodash/merge', '@rematch/core', '@rematch/select'], factory) :
  (global = global || self, factory(global['react-mothership'] = {}, global.React, global.reactRouter, global.reactRedux, global.context$2, global.scope, global._mapValues, global.history, global.store, global._typeof, global._partition, global.path, global.router, global.reactRouterDom, global._get, global._merge, global.core, global.selectPlugin));
}(this, function (exports, React, reactRouter, reactRedux, context$2, scope, _mapValues, history, store, _typeof, _partition, path, router, reactRouterDom, _get, _merge, core, selectPlugin) { 'use strict';
  ...
  function enhance(model,inject){return _objectSpread({},model,{effects:typeof model.effects==='function'?function(){for(var _len=arguments.length,args=new Array(_len),_key=0;_key<_len;_key++){args[_key]=arguments[_key];}return model.effects.apply(model,args.concat([inject]));}:model.effects,selectors:typeof model.selectors==='function'?function(){for(var _len2=arguments.length,args=new Array(_len2),_key2=0;_key2<_len2;_key2++){args[_key2]=arguments[_key2];}return model.selectors.apply(model,args.concat([inject]));}:model.selectors});}function createApp(){var config=arguments.length>0&&arguments[0]!==undefined?arguments[0]:{};var registeredModels={};var store$1=store.createStore(config.rematch);var select=store$1.select,dispatch=store$1.dispatch,getState=store$1.getState;var history$1=history.createBrowserHistory();var inject=_objectSpread({},config.inject,{select:select,dispatch:dispatch,getState:getState});function navigate(views){return _mapValues(views,function(v){return function(){return history$1.push(v.resolve.apply(v,arguments));};});}function registerModels(models,scope){models.forEach(function(model){if(registeredModels[model.name]!==model){var injected=_objectSpread({},inject,{navigator:navigate(scope.views)});registeredModels[model.name]=model;store$1.model(enhance(model,injected));}});}return {store:store$1,history:history$1,registerModels:registerModels,navigate:navigate};}

  function Mothership(_ref){var _ref$app=_ref.app,app=_ref$app===void 0?createApp():_ref$app,children=_ref.children,rest=_objectWithoutProperties(_ref,["app","children"]);return React__default.createElement(context$2.Provider,{value:app},React__default.createElement(reactRedux.Provider,{store:app.store},React__default.createElement(scope.Scope,{name:"root",basePath:"/",hoist:children,provides:rest},React__default.createElement(reactRouter.Router,{history:app.history},children))));}

  var useNavigator=function useNavigator(){var app=context$2.useApp();var _useScope=scope.useScope(),views=_useScope.views;return React.useMemo(function(){return app.navigate(views);},[app,views]);};
  ...
  function createRoutes(basePath,views){return views.map(function(_ref){var path$1=_ref.path,component=_ref.component,effect=_ref.effect,redirect=_ref.redirect,rest=_objectWithoutProperties(_ref,["path","component","effect","redirect"]);var url=path.pathJoin(basePath,path$1);var render=renderHooks(effect&&reduxHook(effect),redirect&&redirectHook(redirect));return React__default.createElement(reactRouter.Route,Object.assign({key:url,path:url,render:render,component:component},rest));});}function renderHooks(){for(var _len=arguments.length,hooks=new Array(_len),_key=0;_key<_len;_key++){hooks[_key]=arguments[_key];}var used=hooks.filter(Boolean);if(!used.length){return undefined;}return function(){return used.map(function(_obj){return _obj.call();});};}function reduxHook(effect){return function(){return null;};}function redirectHook(payload){var view,params;if(typeof payload==='object'){var to=payload.to,rest=_objectWithoutProperties(payload,["to"]);view=to;params=rest;}else{view=payload;}return function(){return React__default.createElement(router.Redirect,{key:view,view:view,params:params});};}

  function createFeature(){var config=arguments.length>0&&arguments[0]!==undefined?arguments[0]:{};return function(Base){var _ref,_Feature;var name=config.name||Base.displayName||Base.name;function Feature(props){var provides=props.provides,path$1=props.path,exact=props.exact,match=props.match,children=props.children;var basePath=path.pathJoin(match.path,path$1);var routes=useRoutes(name,basePath,config.views);var _usePartition=usePartition(children),_usePartition2=_slicedToArray(_usePartition,2),withPath=_usePartition2[0],nested=_usePartition2[1];var render=function render(){return React__default.createElement(Boundary,{fallback:config.placeholder,recovery:config.recovery},React__default.createElement(scope.Scope,{name:name,basePath:basePath,provides:_objectSpread({},config.provides,provides),hoist:children},React__default.createElement(Base,props,React__default.createElement(reactRouter.Switch,null,routes,withPath),nested)));};return path$1?React__default.createElement(reactRouter.Route,{path:basePath,exact:exact,render:render}):render();}var Wrapper=(_ref=(_Feature=Feature,React.memo(_Feature)),reactRouter.withRouter(_ref));Wrapper.displayName="Feature(".concat(name||'Component',")");Wrapper.WrappedComponent=Base;Wrapper.featureConfig=_objectSpread({},config,{name:name});return Wrapper;};}function useRoutes(name,basePath){var views=arguments.length>2&&arguments[2]!==undefined?arguments[2]:[];return React.useMemo(function(){return createRoutes(basePath,views);},[basePath]);}function usePartition(children){return React.useMemo(function(){return _partition(React.Children.toArray(children),function(_obj){return _obj.props.path;});},[children]);}
  ...
  var NotProvidedInScopeError=function(_Error){_inherits(NotProvidedInScopeError,_Error);function NotProvidedInScopeError(path,scope){var _this;_classCallCheck(this,NotProvidedInScopeError);_this=_possibleConstructorReturn(this,_getPrototypeOf(NotProvidedInScopeError).call(this,"Path \"".concat(path,"\" not provided by feature \"").concat(scope,"\"")));_this.name='ViewNotFoundError';return _this;}return NotProvidedInScopeError;}(_wrapNativeSuper(Error));var context=React.createContext({name:'root',views:{},provides:{}});var useScope=function useScope(){return React.useContext(context);};var useProvided=function useProvided(){var scope=React.useContext(context);for(var _len=arguments.length,deps=new Array(_len),_key=0;_key<_len;_key++){deps[_key]=arguments[_key];}return deps.map(function(path){var val=_get(scope.provides,path);if(val===undefined){throw new NotProvidedInScopeError(path,scope.name);}return val;});};

  function Scope(_ref){var basePath=_ref.basePath,hoist=_ref.hoist,children=_ref.children,rest=_objectWithoutProperties(_ref,["basePath","hoist","children"]);var parent=useScope();var views=useViews(basePath,hoist);var scope=_merge({views:views},parent,rest);useModels(hoist,scope);return React__default.createElement(context.Provider,{value:scope,children:children});}Scope.displayName='Scope';function useViews(basePath,hoisted){return React.useMemo(function(){var views={};React.Children.forEach(hoisted,function(child){var _child$type$featureCo,_child$type$featureCo2;var featureName=(_child$type$featureCo=child.type.featureConfig)===null||_child$type$featureCo===void 0?void 0:_child$type$featureCo.name;var featureViews=(_child$type$featureCo2=child.type.featureConfig)===null||_child$type$featureCo2===void 0?void 0:_child$type$featureCo2.views;var featurePath=path.pathJoin(basePath,child.props.path);if(featureName){path.addLinks(views,featurePath,[{name:featureName,exact:child.props.exact,path:'/'}]);}if(featureViews){path.addLinks(views,featurePath,featureViews);}});return views;},[basePath,hoisted]);}function useModels(hoisted,scope){var app=context$2.useApp();React.useMemo(function(){return React.Children.forEach(hoisted,function(child){var _child$type$featureCo3;var featureModels=(_child$type$featureCo3=child.type.featureConfig)===null||_child$type$featureCo3===void 0?void 0:_child$type$featureCo3.models;if(!featureModels){return;}app.registerModels(featureModels,scope);});},[app,hoisted]);}
  ...
  var withDispatch=function withDispatch(_arg){return reactRedux.connect(null,_arg);};var useAppEffect=function useAppEffect(mapDispatch,watch){var app=context$2.useApp();React.useEffect(function(){mapDispatch(app.store.dispatch);},watch);};var sconnect=function sconnect(mapSelect,mapDispatch){return function(Base){var c=React.memo(function(props){var app=context$2.useApp();var enhance=reactRedux.connect(app.store.select(mapSelect),mapDispatch);return React.createElement(enhance(Base),props);});c.displayName="sconnect(".concat(Base.displayName||Base.name||'Component',")");return c;};};var $$=sconnect;var _$=withDispatch;

  var preset = {plugins:[selectPlugin()]};

  var createStore = (function(config){return core.init(_objectSpread({},preset,config));});

  var context$1=React.createContext(null);var useApp=function useApp(){return React.useContext(context$1);};var Provider=context$1.Provider;
